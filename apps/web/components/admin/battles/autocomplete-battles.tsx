// `<BattleAutocomplete />` — debounced combobox for picking a battle by
// Indonesian or Arabic name. Used by the Timeline panel of the figure
// edit form (admin links a figure to a battle via battle_participants).
...
export interface BattleAutocompleteProps {
  value: BattleOption | null
  onChange: (next: BattleOption | null) => void
  placeholder?: string
  id?: string
  invalid?: boolean
  disabled?: boolean
...
export function BattleAutocomplete({
  value,
  onChange,
  ...
  invalid = false,
  disabled = false,
}: BattleAutocompleteProps) {
  const [query, setQuery] = React.useState('')
  const [debounced, setDebounced] = React.useState('')
  const [options, setOptions] = React.useState<BattleOption[]>([])
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    apiPaginated<BattleRow>(`/battles?q=${encodeURIComponent(debounced)}&perPage=${MAX_RESULTS}`)
      .then((res) => {
        if (cancelled) return
        const opts = (res.rows ?? []).slice(0, MAX_RESULTS).map<BattleOption>((r) => ({
          id: r.id,
          slug: r.slug,
          nameId: r.nameId,
          nameAr: r.nameAr,
          eventDateAh: r.eventDateAh,
          eventDateCe: r.eventDateCe,
        }))
        setOptions(opts)
        setHighlight(0)
      })
  ...
  function pick(opt: BattleOption) {
    onChange(opt)
    setQuery('')
    setOpen(false)
    setOptions([])
  ...
  function dateLabel(opt: { eventDateAh: number | null; eventDateCe: number | null }) {
    const parts: string[] = []
    if (typeof opt.eventDateAh === 'number') parts.push(`${opt.eventDateAh} H`)
    if (typeof opt.eventDateCe === 'number') parts.push(`${opt.eventDateCe} M`)
    return parts.join(' / ')
  }
    ...
    return (
      <div className="flex items-center gap-2 rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--bg-elevated))] px-3 py-2">
        ...
        <button
          type="button"
          onClick={clear}
          aria-label="Hapus pilihan peristiwa"
          disabled={disabled}
        ...
        >
          {options.length === 0 ? (
            ...
            options.map((opt, idx) => (
              ...
              >
                <span className="truncate font-medium">{opt.nameId}</span>
                {opt.nameAr ? (
                  <span
                    dir="rtl"
                    lang="ar"
